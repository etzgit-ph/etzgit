...
describe('AgentService orchestration', () => {
  it('runs planner -> llm -> executor successfully', async () => {
    const mockPlanner: any = { determineNextAction: jest.fn().mockResolvedValue([{ filePath: 'README.md', currentContent: '', goal: 'x' }]) };
    const mockLLM: any = { generatePatch: jest.fn().mockResolvedValue([{ filePath: 'README.md', proposedContent: 'New', rationale: 'Improve' }]) };
    const mockExecutor: any = { executeUpgrade: jest.fn().mockResolvedValue(true) };

    const svc = new AgentService(mockPlanner as PlannerService, mockLLM as LLMService, mockExecutor as ExecutorService);
