...
describe('SecretTokenGuard', () => {
  ...
  const mod = await Test.createTestingModule({ providers: [SecretTokenGuard, { provide: 'ConfigService', useValue: mockConfig }, { provide: (require('@nestjs/config').ConfigService), useValue: mockConfig }, { provide: SecurityAuditService, useValue: mockAudit }] }).compile();
    const guard = mod.get(SecretTokenGuard);
    const ctx: any = { switchToHttp: () => ({ getRequest: () => ({ headers: { 'x-agent-secret-key': 's3cr3t' } } ) }) };
  ...
  const mod = await Test.createTestingModule({ providers: [SecretTokenGuard, { provide: 'ConfigService', useValue: mockConfig }, { provide: (require('@nestjs/config').ConfigService), useValue: mockConfig }, { provide: SecurityAuditService, useValue: mockAudit }] }).compile();
    const guard = mod.get(SecretTokenGuard);
    const ctx: any = { switchToHttp: () => ({ getRequest: () => ({ headers: { authorization: 'Bearer bearer-token-1' } } ) }) };
    expect(guard.canActivate(ctx as ExecutionContext)).toBe(true);
  });
});
