...
# 🤖 Autonomous Code Agent (ACA) Monorepo
...
## 🛡️ Core architecture & zero-cost security model
...
- `ci.yml` — mandatory quality gate: lint, tests, coverage, and security scans
- `staging.yml` — ephemeral environment for running E2E against a provisioned service (placeholder Terraform)
- `production.yml` — manual human-triggered deployment
...
## 🗂️ Implementation prompts & alignment
...
- App modules & services (Prompts 061-090): `apps/api` is scaffolded and contains key controllers and modules. I added an `.env.example` and CI/workflow placeholders. Many of the detailed service implementations (Git client hardening, LLM parsing with Zod, executor rollback hooks, Octokit PR wiring) are described in `docs/implementation-prompts.md` but implemented only as skeletons/placeholders in the codebase — this is intentional to keep changes low-risk.
